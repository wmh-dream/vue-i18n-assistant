  ...
  <div class="container p-12">
    <div class="inspection-detail p-12">
      <div class="inspection-detail-info">
        ...
        <p class="mb-12">
          {{ state.onlineInspectionDetails.employeeOrganization }}·{{
            state.onlineInspectionDetails.employeeJob
          }}
        </p>
      </div>
      <div class="inspection-detail-info">
        ...
        <p class="mt-8 mb-8">
          {{ state.onlineInspectionDetails.regionName
          }}{{ state.onlineInspectionDetails.shopAddress }}
        </p>
        <span
          >{{ $t("时间") }}：{{ state.onlineInspectionDetails.auditTime }}</span
        >
        <span
          >{{ $t("时长") }}：{{
            state.onlineInspectionDetails.auditDuration
          }}</span
        >
      </div>
    </div>
    <div class="inspection-detail p-12 mt-12">
      <div class="inspection-detail-info">
        <h3 class="font-12">
          {{ $t("巡查得分")
          }}<b class="font-32 mb-4">{{
            state.onlineInspectionDetails.auditSource
          }}</b>
        </h3>
        <span class="mt-8">
          <i
            >{{ $t("总分")
            }}{{ state.onlineInspectionDetails.totalAuditSource }}</i
          >
          <i
            >{{ $t("检查问题数")
            }}{{ state.onlineInspectionDetails.totalProjectCount }}</i
          >
          <i
            >{{ $t("不合格")
            }}{{ state.onlineInspectionDetails.noQualifyProjectCount }}</i
          >
        </span>
      </div>
    ...
    <div class="question-list">
      ...
      <div
        v-for="item in state.onlineInspectionDetails &&
        state.onlineInspectionDetails.workOrderProjectVoList"
        :key="item.workOrderProjectId"
        class="question-list-item p-12"
      >
        <div class="total-num flex font-12">
          <p>
            <span>{{ $t("总分") }}{{ item.totalSource }}</span
            >｜<span
              >{{ $t("扣分")
              }}{{
                item.deductSource && item.deductSource.replace("-", "")
              }}</span
            >
          </p>
          <label class="flex question-status">{{
